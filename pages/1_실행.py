...
if src == "등록된 데이터셋":
    ...
    if st.button("적재 시작", type="primary"):
        ...
        run = st.session_state.run
        st.session_state.pop("uploaded", None)   # 업로드본을 버리고 등록본으로 돌아간다
    ...
    st.caption("올린 파일은 **등록된 데이터셋 위에 덮어씁니다.** 같은 이름의 테이블만 "
               "바뀌고 나머지는 그대로입니다 — 파일 하나만 올려 검증을 시험할 수 있습니다. "
               "배포본에서는 파일당 20MB로 제한됩니다.")
    if up and st.button("적재 시작", type="primary"):
        st.session_state.run = gates.new_run("업로드")
        run = st.session_state.run
        # ⚠ 지역변수에만 담으면 아래 st.rerun() 뒤에 사라져 data/ 를 다시 읽는다.
        #   그러면 무엇을 올리든 원본이 검증된다. 세션에 남겨야 한다.
        st.session_state.uploaded = load.load_uploaded(up)
        gates.log(run, "업로드 덮어쓰기: "
                       + ", ".join(sorted(st.session_state.uploaded)))
        gates.advance(run, "적재")
        st.session_state.loaded = True
        st.rerun()
...
if tables is None:
    ...
    if tables is None:
        st.stop()
    # 업로드본이 있으면 같은 이름의 테이블만 갈아끼운다
    over = st.session_state.get("uploaded")
    if over:
        tables = {**tables, **over}
        st.warning("업로드본으로 덮어쓴 테이블: **" + "**, **".join(sorted(over))
                   + "** — 등록된 데이터셋이 아닙니다.")
...
if gates.is_passed(run, 1):
    ui.section("4. 계산")
    f = ui.guard(M.funnel, tables["funnel_events_synthetic"])
